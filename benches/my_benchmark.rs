...
fn bench_rbtree_insertion(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("rbtree_test_insertion");
    for size in [10000, 40000, 70000, 100000, 130000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |bench, &size| {
            bench.iter(|| {
                let mut tree: RBTree<i32> = rbtree::RBTree::new();
...
fn bench_avl_tree_insertion(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("avltree_test_insertion");
    for size in [10000, 40000, 70000, 100000, 130000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |bench, &size| {
            bench.iter(|| {
                let mut tree: AVLTree<i32> = avltree::AVLTree::new();
...
fn bench_avl_tree_search(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("avltree_test_search");
    let mut tree: AVLTree<i32> = avltree::AVLTree::new();
    for index in 1..130000 {
        tree.insert(index)
    }
    for size in [10000, 40000, 70000, 100000, 130000, 200000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |bench, &size| {
            bench.iter(|| {
                for index in 1..size/10 {
                    tree.contain(index);
                }
            })
        });
...
fn bench_rbtree_search(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("rbtree_test_search");
    let mut tree: RBTree<i32> = rbtree::RBTree::new();
    for index in 1..130000 {
        tree.insert(index)
    }
